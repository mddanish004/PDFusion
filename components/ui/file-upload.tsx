...
import { Trash, X, CircleAlert, Play, Pause, Upload, FileText, Image, GripVertical } from 'lucide-react'
...
export const FileList: React.FC<FileListProps> = ({
  ...
  const handleReorder = (newFiles: FileInfo[]) => {
    onReorder?.(newFiles)
  }

  const FileItemWithDrag = ({ file }: { file: FileInfo }) => (
    <div className="flex items-center gap-3 p-3 rounded-md border bg-background shadow-sm">
      {canReorder && (
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto">
        {canReorder ? (
          <Reorder.Group axis="y" values={files} onReorder={handleReorder}>
            {files.map(file => (
              <Reorder.Item key={file.id} value={file}>
                <FileItemWithDrag file={file} />
              </Reorder.Item>
            ))}
          </Reorder.Group>
        ) : (
...
export const DropZone: React.FC<DropZoneProps> = ({
  ...
  const {
    ...
    appendFiles: contextAppendFiles,
  } = useFileUpload()
  ...
  const appendFiles = contextAppendFiles
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (appendFiles) {
        const newFileInfos = getFileInfos(droppedFiles)
        contextOnFileSelectChange?.([...contextFiles, ...newFileInfos])
      } else {
  ...
  const handleFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (appendFiles) {
        const newFileInfos = getFileInfos(selectedFiles)
        contextOnFileSelectChange?.([...contextFiles, ...newFileInfos])
      } else {
